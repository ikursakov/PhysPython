...
# Определим функции, возвращающие компоненты вектора магнитной индукции $\vec{B}$. При этом воспользуемся методом `scipy.integrated.dblquad()` (double quadrature) для численного расчёта двойного определённого интеграла. Следует обратить внимание, что в `dblquad()` предполагается, что интегрируемая функция $f$ от двух переменных принимает их в порядке $f(\xi_2, \xi_1)$.
